...
export default function ProjectCards() {
  ...
  useEffect(() => {
    const results = projects.filter((project) => {
      ...
      const matchesStatus =
        statusFilter === "all" ||
        normalizeStatus(project.statut) === normalizeStatus(statusFilter);
  ...
  const onDelete = async (id) => {
    if (
      ...
      try {
        await axios.delete(`http://localhost:8080/manageWeb/projets/${id}`, {
          headers: {
            Authorization: `Bearer ${authToken}`,
          },
        });
              ...
              <div className="p-6">
                <div className="flex justify-between items-start mb-4">
                  <h3 className="text-lg font-semibold text-gray-800 truncate">
                    {project.nom || project.description || "Projet sans nom"}
                  </h3>
                  <span
                ...
                <div className="space-y-3 text-sm text-gray-600">
                  <div className="flex items-center">
                    ...
                    <span>Créé: {formatDate(project.dateCreation)}</span>
                  </div>
                  ...
                  {project.dateLivraison && (
                    <div className="flex items-center">
                      ...
                      <span>
                        Livraison: {formatDate(project.dateLivraison)}
                      </span>
                    </div>
                  )}
                </div>
